...
- `src/context/ResponsesContext.tsx` — responses CRUD (`createResponse`, list by template).
...
Keys: `formbuilder:v1:templates`, `formbuilder:v1:responses`, `formbuilder:v1:schemaVersion`. Typed load/save with try/catch + safe fallback. Version key enables future migration. Works after full refresh (contexts hydrate from storage on mount).

### 8. PDF export — `src/pdf/exportPdf.ts` (browser-native only)
Takes a `FormResponse` and renders **its `templateSnapshot`**, not the live `FormTemplate` from `TemplatesContext` — this guarantees the export always matches exactly what was submitted, even if the template has since been edited (§2). Runs the shared pipeline (§5) over `response.values` and `templateSnapshot.fields` to get `getVisibleEntries`'s output, then `window.open()`s a blank document, writes a self-contained styled HTML doc (form title, submission timestamp, each visible entry's label + `formatForDisplay(value)` in form order), and calls `window.print()`. File-upload fields render filenames + a "(file not embedded)" note. A visible **Section Header** is the one exception: it renders as a structural heading (its label, sized per its `size` config) via `renderForPdf`, since it's the only field type that defines that hook instead of `formatForDisplay`. Everything else — including **Calculation**, which is never editable but still has a real computed value — renders as a normal label/value row via `formatForDisplay(computedValue, config)`, using the calculation-merged value `getVisibleEntries` already provided. Reused for both first export and re-download from the responses list (both read the same `response.templateSnapshot`, so re-download is byte-for-byte reproducible regardless of template edits made afterward). Clean typographic layout via inline `<style>` + `@media print`.

### 9. Routing & pages — React Router (`src/pages/`)
- `TemplatesList` (`/`) — cards: title, field count, response count, last modified. Clicking the card body opens `Builder` (edit); each card also has its own "New Response" button (→ creates response, opens `Fill`) and a "View Responses" link (→ `Responses`), so all three destinations are reachable straight from the card without ambiguity about what a plain click does. "New Template" button in the header navigates to `Builder` at **`/builder/new`** (§6 — no template is persisted until first Save).
- `Builder` (`/builder/:templateId | new`) — header bar with an editable **title input** (bound to the local draft's title, spec requires this to show on cards/PDF) plus a link to that template's `Responses` (only shown once the template has been saved at least once — `/builder/new` hasn't got an id to link to yet). 3 panels below: `FieldPalette` (left, click/drag to add), `Canvas` (center, dnd-kit sortable + drop target), `ConfigPanel` (right, selected field's `ConfigPanel` + shared Conditions/Required-toggle/Default-visibility editor) — all three operate on the local `builderReducer` draft (§6), not `TemplatesContext` directly. Save commits the draft via `TemplatesContext.updateTemplate` (or `createTemplate` + redirect to `/builder/:newId` the first time). Preview (modal → Fill). **Preview renders the same `FormRenderer` as real Fill mode over the *draft*, but its Submit just closes the modal (never calls `ResponsesContext`'s create action) and its Download PDF is disabled/hidden** — there's no real `FormResponse` or `submittedAt` to export in preview, so testing a form as a builder can't pollute the real Responses list or produce a PDF with a fabricated timestamp.
- `Fill` (`/fill/:templateId`) — `FormRenderer` maps fields → `FillField` wrapped by visibility/required from the engine; real-time conditions + calculations via the §5 pipeline; validate on submit against `getVisibleEntries` output. Submit builds `FormResponse` with `templateSnapshot: structuredClone(template)`, saves it, and stores it in local state — **it does not immediately navigate away**. **Download PDF lives on the Fill page itself, disabled until that local "just-submitted" response exists** (revised decision — an earlier draft removed it entirely for the same genuine-timestamp reason given for Preview above, but the PDF-export requirement is actually satisfiable here: once Submit has created a real `FormResponse`, its `submittedAt` is genuine, so Download PDF simply stays disabled — with a `title` tooltip explaining why — until that moment, then calls the same `exportResponseToPdf` used by the `Responses` list). After a successful Submit, the page shows a "Submitted ✓" indicator and the now-enabled Download PDF button — no forced redirect and no extra "View Responses" link on this page (that's reachable from the template card and Builder header, which is enough; Fill stays focused on the fill/submit/download loop).
- `Responses` (`/template/:id/responses`) — list submissions with timestamp + Re-download PDF; reachable from the template card and from the Builder header.
...
7. `getVisibleEntries` (§5) + validation on submit; response save with `templateSnapshot`; Responses list + navigation links from card/Builder header.
8. PDF export.
...
- [ ] `src/types/`: `FieldConfig` (discriminated union), `FormField`, `Condition`, `FormTemplate`, `FormResponse`
...
- [ ] `src/context/ResponsesContext.tsx` + `useResponses()` hook
...
- [ ] `TemplatesList` page: New Template button (→ `/builder/new`), card grid (title/field count/response count/last modified)
...
- [ ] `ResponsesContext` create action; `FormResponse` saved with `templateSnapshot: structuredClone(template)`, `values` = `getVisibleEntries` output, and a timestamp
- [ ] `Responses` page: list per template, timestamp, Re-download PDF button
- [ ] Navigation wired: card → New Response → Fill; card → View Responses → Responses; Builder header → Responses. Submit does **not** auto-redirect from Fill — it stays put and shows a "Submitted ✓" state with Download PDF now enabled (no "View Responses" link on Fill itself — reachable from the card/Builder header instead)
- [ ] "New Response" opens `Fill` with fresh/empty local state — no `FormResponse` is created until Submit fires
- [ ] Template `updatedAt` is bumped on every Builder Save

### Step 8 — PDF export
- [ ] `src/pdf/exportPdf.ts`: `window.open` + styled HTML + `window.print()`
- [ ] Renders `response.templateSnapshot` + `response.values`, not the live template from `TemplatesContext` (§8 architecture) — re-download stays correct even if the template was edited afterward
...
- [ ] Re-download from Responses list produces an identical PDF (same `templateSnapshot`, same pipeline)
- [ ] Fill page's own Download PDF button is disabled (with a "Submit the form first" tooltip) until Submit succeeds, then calls the same `exportResponseToPdf` against the just-created response — not a separate export path
- [ ] `@media print` layout smoke-tested: reasonable margins, no awkward page breaks through a field's label/value pair

### Step 9 — Polish
- [ ] Preview modal in Builder (reuses `FormRenderer` over the draft; Submit just closes modal — no `ResponsesContext` write; Download PDF button hidden/disabled in preview since there's no real `submittedAt` to export — §9)
...
| B | 2 | `feat(types): add FormTemplate, FormField, and FieldConfig unions` · `feat(storage): add versioned localStorage helpers` · `feat(context): add thin TemplatesContext (create/update/delete only, no field-editing actions)` · `feat(context): add ResponsesContext and app providers shell` |
...
| G | 7 | `feat(engine): add getVisibleEntries for submit and export` (ships here, ahead of PDF, since Phase H depends on it) · `feat(responses): add templateSnapshot on FormResponse and submit validation` · `feat(pages): add Responses list with timestamps` · `feat(routing): wire New Response, View Responses, post-submit navigation, and updatedAt-on-save` |
| H | 8 | `feat(pdf): add browser-native export via print HTML, rendering response.templateSnapshot` (uses `getVisibleEntries` + `formatForDisplay`/`renderForPdf` from the start) · `feat(responses): add Re-download PDF from saved submissions` |
| I | 9 | `feat(builder): add Preview modal over the draft, with response persistence and PDF download both disabled` · `feat(ui): add empty states for templates, builder, and responses` · `feat(ui): extract common Button/Input/Label/Card primitives` (only if duplicated 3+ times) · `fix(a11y): add labels, aria-invalid, and keyboard support for selects` |
...
- **Manual flows (optionally via Playwright MCP):**
  ...
  - Submit → appears in Responses list with timestamp.
  ...
  - **Re-download after template edit:** submit a response, then edit the template (rename a field, add a new one), then Re-download that response's PDF from Responses — it must still show the *original* field labels/values from `templateSnapshot`, not the edited template.
...
- Date "pre-fill today" populates on a fresh response; min/max date bounds enforced.
