...
- `Fill` (`/fill/:templateId`) — `FormRenderer` maps fields → `FillField` wrapped by visibility/required from the engine; real-time conditions + calculations via the §5 pipeline; validate on submit against `getVisibleEntries` output. Submit builds `FormResponse` with `templateSnapshot: structuredClone(template)` and redirects to that template's `Responses` list. **No separate "Download PDF" on the Fill page itself** (documented decision): the PDF-export section requires the exported PDF to include a genuine submission timestamp, which doesn't exist until Submit has actually created a `FormResponse` — same reasoning already applied to Preview's disabled Download PDF above. Download PDF lives only on the `Responses` list, immediately reachable after Submit's redirect.
...
- [ ] Navigation wired: card → New Response → Fill → Submit → Responses; card → View Responses; Builder header → Responses
